...
function SymptomIcon({ variant }: { variant: SymptomIconVariant }) {
  if (variant === 'dot-orange') {
    return <span className="inline-block size-3 shrink-0 rounded-full bg-[#f59040]" aria-hidden />;
  }
  if (variant === 'dot-gray') {
  ...
  return <Leaf className="size-5 shrink-0 text-[#f59040]" strokeWidth={2} aria-hidden />;
}
    ...
    <CardSurface className="h-full flex flex-col gap-3">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <h3 className="text-[#f59040] font-semibold text-lg leading-snug">{title}</h3>
